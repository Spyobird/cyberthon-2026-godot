...
signal scene_transition_request_started(new_scene: String, effect: TransitionEffect, duration: float)
...
func _on_scene_transition_request_started(new_scene: String, effect: TransitionEffect, duration: float) -> void:
	await _play_transition(new_scene, effect, duration)

# ---------------------------------------------------------------------------
# Transition dispatch
# ---------------------------------------------------------------------------

func _play_transition(new_scene: String, effect: TransitionEffect, duration: float = 2.0) -> void:
	match effect:
		...
		TransitionEffect.DIAGONAL_POPPING_SQUARES:
			await _play_shader_transition(new_scene, {
				"transition_type": 0,
				"grid_size": Vector2(35.5, 20.0),
				"position": Vector2(0.5, 0.5),
				"progress_bias": Vector2(3.0, 1.7),
				"_p_transparent": 0.0,
				"_p_opaque": 14.5,
				"invert": true
			}, duration)
...
func _play_shader_transition(new_scene: String, params: Dictionary, duration: float = 2.0) -> void:
	...
	tween.tween_method(
		...
		p_transparent, p_opaque, duration
	)
	...
	tween.tween_method(
		func(v: float) -> void: _shader_material.set_shader_parameter("progress", v),
		p_opaque, p_transparent, duration
	)
	await tween.finished
